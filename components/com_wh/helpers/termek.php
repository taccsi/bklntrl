...
class xmlTermek extends kep{
	...
	function getTermekTipus( $node){
		...
		$ret = "";
		foreach( array( "", "DARABARU", "CSOMAGOLT_ARU", "TEKERCSES_ARU", "METERES_TERMEKEK" ) as $a ){ 
			$o="";
			$o->value = $a;
			$o->option = jtext::_($a);			
			$arr[]=$o;
...
<?php echo $obj->nev ?>
   ...
   function getKapcsolodoTermekKategoria($node){
   		ob_start();
		$name = $node->getAttribute('name');
		$value = $this->getaktVal($name);
		$q = "select id as `value`, nev as `option` FROM #__wh_gyarto";
		$this->db->setQuery($q);
		$rows = $this->db->loadObjectList();
		//$value = jrequest::getvar('fotermek');
		echo "<input type=\"hidden\" id=\"{$name}\" readonly=\"readonly\" value=\"132\" name=\"{$name}\" />";
		$ret = ob_get_contents();
		ob_end_clean();
		return $ret;
   }
   
    function getKapcsolodoTermekIdFromUrl($node){
   		ob_start();
		$name = $node->getAttribute('name');
		$value = $this->getaktVal('kapcsolodo_termek_id');
		if (!$value){$value = jrequest::getvar('kapcsolodo_termek_id');}//
		echo "<input type=\"hidden\" id=\"{$name}\" readonly=\"readonly\" value=\"{$value}\" name=\"{$name}\" />";
		$ret = ob_get_contents();
		ob_end_clean();
		return $ret;
   }
	function getKapcsolodoTermekek( $node ){
		...
		$cid = jrequest::getvar('cid');
		$termek_id = $cid[0];
		$ret = "<br /><a onclick = \"window.open('index.php?option=com_wh&controller=termek&task=edit&cid[0]=&fromlist=1&kapcsolodo_termek_id={$termek_id}&layout=kapcsolodo','Új kapcsolódó szolgáltatás','width=800,height=600')\" href=\"javascript:;\">Új kapcsolódó szolgáltatás felvitele</a><br />";
		$ret .= Jtext::_('MAR_MEGLEVO_KAPCSOLODO_HOZZAADAS');
		$ret .= "<input type=\"text\" id=\"{$autoCName}\" name=\"{$autoCName}\" />";
		//$ret .= "<input type=\"hidden\" id=\"{$name}\" name=\"{$name}\" value=\"\" />";		
