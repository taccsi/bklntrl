...
class whModelgalleries extends modelBase {
	function __construct() {
		...
		$this -> xmlParser = new xmlParser("gallery.xml");
	}//function

	function _buildQuery() {
		$cond = $this -> getCond();
		$query = "SELECT * FROM #__wh_gallery as g {$cond} order by g.nev";
		//echo $query;
		return $query;
	}
	...
	function setLink($item) {
		global $Itemid;
		$ret = "";
		$link = JRoute::_("index.php?option=com_wh&controller=gallery&task=show&fromlist=1&Itemid={$Itemid}&cid[]={$item->id}");
