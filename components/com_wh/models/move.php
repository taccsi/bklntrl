...
class whModelmove extends modelbase{
	function expKategoriak(){
		$q = "select * from bikeline_temp.bl_vm_category as c inner join bikeline_temp.bl_vm_category_xref as xc
	...
	function expTermekek(){
		//die("**");
		$q = "select * from bikeline_temp.bl_vm_product as p 
		left join bikeline_temp.bl_vm_product_category_xref as xp on p.product_id = xp.product_id
		left join bikeline_temp.bl_vm_product_price as price on p.product_id = price.product_id
	...
	function letrehozKep( $termek_id, $p="", $csoport="", $nev = "", $leiras = "" ){
		$arr = array();
		$arr[]="../temp/components/com_virtuemart/shop_image/product/{$p->product_full_image}";
		$q = "select concat('..', file_name ) from bikeline_temp.bl_vm_product_files where file_product_id = {$termek_id} and file_mimetype = 'image/jpeg' ";
		$this->_db->setQuery($q);
	...
	function letrehozLencsek( $termek_id, $vm_termek ){
		...
		foreach( $szinCikkszamArr as $c ){
			$p = $this->getObj( "bikeline_temp.bl_vm_product", $c, "product_sku" );
		...
		foreach( $idArr as $c ){
			if(trim($c)){
				...
				if( !$t_ ){
					...
					$forrasKep = "../temp/components/com_virtuemart/shop_image/product/{$p->product_full_image}";
