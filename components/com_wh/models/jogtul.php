...
class whModeljogtul extends modelbase
{
	var $xmlFile = "jogtul.xml";
	var $uploaded = "";
	var $tmpname = "";
	var $table = "#__wh_jogtul";
	...
	function __construct()
	{
		...
		$this->getData();
	 	$this->xmlParser = new xmljogtul($this->xmlFile, $this->_data);
	...
	function csvExport(){
		...
		$h =";";
		$fp = fopen($filename, "w");
		$q = "select * from #__wh_jogtul as jogtul order by nev ";
		$this->_db->setQuery( $q );
		...
		foreach( $this->_db->loadObjectList() as $sz){
			if( $arr = $this->getJogtulKimutatasExport(0, $sz->id ) ){
				$line = "";
				foreach($fej1 as $f1){
					$line .= "{$elv}".$sz->$f1."{$elv};";
				}
	...
	function getJogtulKimutatasExport($cellspacing = 0, $jogtulajdonos_id = 0 ){
		ob_start();
		//$link = "index.php?option=com_wh&controller=jogtul&task=jutalekNyomtatas&fromlist=1&jogtulajdonos_id={$this->_data->id}&cid[]={$this->_data->id}&cond_ev";
		echo $this->_db->getErrorMsg();
		$cond_ev = $this->getValtozok()->cond_ev;
		$cond_honap = $this->getValtozok()->cond_honap;
		( $jogtulajdonos_id ) ? $jogtulajdonos_id : $jogtulajdonos_id = $this->getValtozok()->jogtulajdonos_id;
		//die("{$jogtulajdonos_id}");
		$q = "select replace(termek.isbn, '-', '') as isbn from #__wh_termek as termek
		where termek.szerzo_id in (select szerzo_id from #__wh_jogtul_kapcsolo where jogtulajdonos_id = {$jogtulajdonos_id} ) ";
		$q = "select replace(termek.isbn, '-', '') as isbn from #__wh_termek as termek
		...
		(select szerzo_id from #__wh_jogtul_kapcsolo where jogtulajdonos_id = {$jogtulajdonos_id} ) ";		
		...
		$osszJutalek = 0;
		foreach($rows as $r){
			$tulhanyad = $this->getTulhanyad($r->szerzo_id, $jogtulajdonos_id);
	...
	function getJogtulKimutatas($cellspacing = 0, $jogtulajdonos_id = 0 ){
		ob_start();
		//$link = "index.php?option=com_wh&controller=jogtul&task=jutalekNyomtatas&fromlist=1&jogtulajdonos_id={$this->_data->id}&cid[]={$this->_data->id}&cond_ev";
		echo $this->_db->getErrorMsg();
		$cond_ev = $this->getValtozok()->cond_ev;
	...
	function tomegesEmailKuldes(){
		...
		$q = "select id from #__wh_jogtul as jogtul 
	...
	function kuldErtesitoEmail( $jogtulajdonos_id = 0 ){
		ob_start();
		//echo $this->_db->getErrorMsg();
		if($kimutatas = str_replace("&nbsp;", "", $this->getJogtulKimutatas( 10, $jogtulajdonos_id ) ) ){
			$cond_ev = $this->getValtozok()->cond_ev;
			$cond_honap = $this->getValtozok()->cond_honap;
			( $jogtulajdonos_id ) ? $jogtulajdonos_id : $jogtulajdonos_id = $this->getValtozok()->jogtulajdonos_id;		
			$q = "update #__wh_jogtul set datum = now() where id = {$jogtulajdonos_id}";
			$this->_db->setQuery($q);
			$this->_db->Query();
			$em_ = new xCemail;
			$from = "info@dsffds.hu";
			$fromname = "fdsfsf.hu";		
			(array)$recipient[]=$this->getObj("#__wh_jogtul", $jogtulajdonos_id )->email;
			//$recipient[]="tamas@trifid.hu";
			$subject = jtext::_("ELSZAMOLAS");
			$footer = jtext::_("ELSZAMOLAS_FOOTER");
			$header = "<h1>".jtext::_("KEDVES")." ".$this->getObj("#__wh_jogtul", $jogtulajdonos_id )->nev."!</h1>";
			$mode =1;
	...
	function getEmailElkuldve(){
		ob_start();
		$jogtulajdonos_id = $this->getValtozok()->jogtulajdonos_id;		
		$q = "select datum from #__wh_jogtul
		where id = {$jogtulajdonos_id}";
		$this->_db->setQuery($q);
	...
	function getJogtulszemelyesAdatok( $jogtulajdonos_id ){
		//$nev = $this->getObj(,  )->nev;	
		$o = $this->getObj("#__wh_jogtul", $jogtulajdonos_id );
		$ret = "";
		@$ret .= "<h2>{$o->nev}</h2>";
	...
	function getNyomtatasCim(){
		...
		@$ret .= $this->getJogtulszemelyesAdatok( $this->getValtozok()->jogtulajdonos_id);
		@$ret .="<h3>".jtext::_("ELSZAMOLASI_IDOSZAK").": {$cond_ev} {$this->negyedevekVissza[$cond_honap]}. ".jtext::_("NEGYEDEV")."</h3>";		
		return $ret;
	}
	
	function getJogtulKimutatasTomeges(){
		//print_r( $this->getValtozok() );
		$q = "select id from #__wh_jogtul as jogtul 
		where email = '' order by nev ";
		$this->_db->setQuery( $q );
		...
		@$cond_honap = $this->getValtozok()->cond_honap;		
		foreach( $this->_db->loadResultArray() as $id){
			if( $kimutatas = str_replace("&nbsp;", "", $this->getJogtulKimutatas( 10, $id ) ) ){
				$ret .= $this->getJogtulszemelyesAdatok( $id );
	...
	function getTulhanyad($szerzo_id, $jogtulajdonos_id){
		$q = "select tulhanyad from #__wh_jogtul_kapcsolo where szerzo_id = {$szerzo_id} and jogtulajdonos_id = {$jogtulajdonos_id}";
		$this->_db->setQuery($q);
		return $this->_db->loadResult();
	}
	
	function getValtozok(){
		$arr = array("cond_ev", "cond_honap", "jogtulajdonos_id");
		$ret ="";
		foreach($arr as $a){
	...
	function getTelepules(){
		...
		$id = jrequest::getVar("jogtul_id", 0);	
		//echo $id." -------------";
		if( $id ){	
			$obj = $this->getObj( "#__wh_jogtul", $id );
			//print_r($obj);
