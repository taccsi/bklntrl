...
<div class="div_wh_top_holder">
  <h3 class="h3_contentheading"><?php echo JText::_("Szerzők") ?></h3> 
...
<?php
	...
	if(count( $this->items )){
		...
		for ($i=0, $n=count( $this->items ); $i < $n; $i++)
		{
			...
			$link = JRoute::_( "index.php?option=com_wh&controller=szerzo&task=edit&fromlist=1&Itemid={$Itemid}&cid[]={$row->id}" );
			$o = "";
			$o->CHECKED = $checked;
			$o->szerzo = "<a href=\"{$link}\">{$row->nev}</a>";
			$arr[] = $o; 
			$k = 1 - $k;
		}
...
<input type="hidden" name="controller" value="szerzok" />
<?php
