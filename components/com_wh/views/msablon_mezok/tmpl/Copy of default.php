...
<?php echo $this->search ?>
    ...
    <tr>
        <th colspan="6"><?php echo JText::_("MSABLON_MEZO NEV") ?></th>
        <th></th>
    </tr>
	<?php
	if(count( $this->items )){
		...
		for ($i=0, $n=count( $this->items ); $i < $n; $i++)
		{
			...
			$tmpl = ($tmpl) ? '&tmpl='.$tmpl : '';
			$link = JRoute::_( 'index.php?option=com_wh&controller=msablon_mezo&task=show&fromlist=1&Itemid='.$Itemid.$tmpl.'&cid[]='. $row->id );
			?>
