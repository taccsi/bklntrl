...
<div class="div_wh_top_holder">		
    <h3 class="h3_contentheading"><?php echo JText::_("MSABLONOK") ?></h3>    
...
<?php
	...
	if(count( $this->items )){
		...
		for ($i=0, $n=count( $this->items ); $i < $n; $i++)
		{
			...
			$tmpl = ($tmpl) ? '&tmpl='.$tmpl : '';
			$link = JRoute::_( 'index.php?option=com_wh&controller=msablon&task=show&fromlist=1&Itemid='.$Itemid.$tmpl.'&cid[]='. $row->id );
			$o = "";
			$o->CHECKED = $checked;
			$o->MUSZAKI_SABLONOK = "<a href=\"{$link}\">{$row->nev}</a>";
			$arr[] = $o; 
			$k = 1 - $k;
		}
...
<input type="hidden" name="controller" value="msablonok" />
<?php 
