...
class xmlKategoria extends xmlParser{
	...
	function getSorrend($node){
		...
		$q = "select count(id) as sorrend from #__whp_kategoria where szulo = {$szulo} and id <> {$id}";
		$this->db->setQuery($q);
   ...
   function getSzuloKategoria($node) {
	  	...
	  	$id = $this->getAktVal("id");
	  	if(!$id) $id = 0;
	  	$kategoriafa = new kategoriafa( array($id) );
	  	$arr = array();
		$o="";
		$o->option=$o->value="";
   ...
   function getSablon($node){
   	  	$name = $node->getAttribute('name');
      	$value=$this->getAktVal($name);
	  	$q = "select id as `value`, nev as `option` from #__whp_msablon";
		$this->db->setQuery($q);
