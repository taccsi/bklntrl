...
class xmlRecommender extends xmlParser{
	...
	function getFeladButton($node){
		...
		?>
        <input onclick="\$j('#termek_id').val('<?php echo $termek_id ?>');  addUzenet('<?php echo $controller ?>');" name="<?php echo $name ?>" value="<?php echo Jtext::_($name); ?>" type="button" /><?php
		$ret = ob_get_contents();
		ob_end_clean();
		return $ret;   				
	}
