...
class kep extends xmlParser{
   ...
   function getKepNev($id){
	...
	function upload(){ 
		...
		$q = "select sorrend from #__whp_kep where termek_id = {$o->termek_id} order by sorrend desc limit 1";
	...
	function image($forras_kep, $cel_kep, $link="", $w="", $h="", $mode="", $class="", $buborek_kep="", $alt=""){
		...
		if(file_exists($cel_kep)){
			...
			if(!$buborek_kep){ ?>
                ...
                <img alt="<?php echo $alt ?>" title="<?php echo $alt ?>" class="kiskep" src="<?php echo $cel_kep."?r={$rand}" ?>" /></a>
                ...
                <a href="<?php echo $link /*."&r={$rand}"*/ ?>" <?php echo $class ?> title="<?php echo $buborek_kep ?>" >

                <img class="kiskep" alt="<?php echo $alt ?>" src="<?php echo $cel_kep."?r={$rand}" ?>" /></a>
