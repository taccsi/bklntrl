...
<?php
	...
	if(count( $this->items )){
		...
		for ($i=0, $n=count( $this->items ); $i < $n; $i++)
		{
			...
			$tmpl = ($tmpl) ? '&tmpl='.$tmpl : '';
			$link = JRoute::_( 'index.php?option=com_whp&controller=szuz&task=edit&fromlist=1&Itemid='.$Itemid.$tmpl.'&cid[]='. $row->id );
			$o = "";
			$o->CHECKED = $checked;
...
<input type="hidden" name="controller" value="szuzek" />
</form>