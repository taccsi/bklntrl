...
class whpModelrendelesek extends whpAdmin
{
	...
	function minta(){
		for($i = 0; $i<100 ; $i++ ){
			...
			$o->aktiv = "igen";			
			$this->_db->insertObject("#__whp_rendeles", $o, "id");
		}
	}
	...
	{
		$cond = $this->getCond();	
		$query = "SELECT rendeles.* FROM #__whp_rendeles as rendeles {$cond}";
		return $query;
	}
	...
	function setVasarlo($item){
		parse_str($item->felhasznaloi_adatok);
		$item->vasarlo = "{$name} {$email} {$telefon}";
		return $item; 
	}
