...
class whpModelmsablonok extends whpAdmin
{
	...
	function minta(){
		for($i = 0; $i<100 ; $i++ ){
			...
			$o->aktiv = "igen";			
			$this->_db->insertObject("#__whp_msablon", $o, "id");
		}
	}
	...
	{
		$cond = $this->getCond();	
		$query = "SELECT msablon.* FROM #__whp_msablon as msablon {$cond}";
		return $query;
	}
	...
	function getData()
	{
		...
		if (empty( $this->_data ))
		{
			...
			//$this->_data = array_map ( array($this, "setmsablonok"), $this->_data) ;			
